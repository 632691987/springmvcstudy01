...
public abstract class AbstractAspect {
	...
	protected <T> T getParameterValue(Signature signature, Object[] values, String parameterName, Class<T> clazz) {
		...
		if (index == -1)
			throw new IllegalArgumentException(String.format("Operation \"%s\" doesn't have a parameter called \"%s\"!",
					signature.getName(), parameterName));
		return getCastedValueOrThrowException(values, index, methodSignature.getName(), parameterName, clazz);
	}

	@SuppressWarnings("unchecked")
	private <T> T getCastedValueOrThrowException(Object[] values, int index, String operation, String parameter,
                                                 Class<T> clazz) {
		try {
			return (T) values[index];
		} catch (final ClassCastException e) {
			throw new ClassCastException(
					String.format("Operation \"%s\" has a parameter called \"%s\" but its type is %s, not %s!",
							operation, parameter, values[index].getClass().getSimpleName(), clazz.getSimpleName()));
		}
	}
}